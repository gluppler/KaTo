...
-- Display tokens with counts (Monads, List Processing, Higher-Order Functions)
displayTokens :: [String] -> IO ()
displayTokens tokens = do     -- IO Monad to handle side effects (printing to console)
    putStrLn $ "Token Count: " ++ show (length tokens)     -- Output total number of tokens
    let uniqueTokens = List.nub tokens                     -- List processing to remove duplicates
    putStrLn $ "Unique Tokens: " ++ show (length uniqueTokens)  -- Output number of unique tokens
    putStrLn "Tokens: "
    mapM_ putStrLn uniqueTokens                            -- Iterate over unique tokens and print each one (using mapM_ for IO action)
